...
DETAILS_SHARD_DIR = File.join(OUTPUT_DIR, "entry-details")
DETAILS_SHARD_LOADER_FILE = File.join(OUTPUT_DIR, "entry-detail-shards.generated.ts")
...
def entry_detail_shard_key(entry)
  key = entry["letter"].to_s.strip.downcase
  key.match?(/\A[a-z]\z/) ? key : "other"
end

def entry_detail_shard_loader_source(shard_keys)
  entries = shard_keys.map do |key|
    "  #{JSON.generate(key)}: () => import(#{JSON.generate("./entry-details/#{key}.json")}),"
  end.join("\n")

  <<~TS
    // This file is generated by scripts/generate-content-index-fallback.rb.
    // Do not edit by hand.
...
def create_entry_reference_resolver(entries)
  entry_by_slug = {}
  entry_by_title = {}
  entry_by_alias = {}
  ...
  entries.each do |entry|
    ...
    entry_by_title[entry["title"].strip.downcase] = entry
    Array(entry["aliases"]).each do |aliaz|
      key = aliaz.strip.downcase
      entry_by_alias[key] ||= entry unless key.empty?
    end
  end

  lambda do |reference, exclude_slug: nil|
    trimmed = reference.to_s.strip
    next nil if trimmed.empty?

    normalized = trimmed.downcase
    match =
      entry_by_slug[trimmed] ||
      entry_by_slug[slugify(trimmed)] ||
      entry_by_title[normalized] ||
      entry_by_alias[normalized]

    next nil if match && match["slug"] == exclude_slug

    match
  end
end

def resolved_entry_references(entry, field, resolver)
  Array(entry[field]).map do |label|
    match = resolver.call(label, exclude_slug: entry["slug"])
    if match
      { "label" => label, "entrySlug" => match["slug"] }
    else
      { "label" => label }
    end
  end
...
resolve_entry_reference = create_entry_reference_resolver(raw_entries)
...
entries = raw_entries.map do |entry|
  ...
  enriched["resolvedSeeAlso"] = resolved_entry_references(entry, "seeAlso", resolve_entry_reference)
  enriched["resolvedVendorReferences"] = resolved_entry_references(entry, "vendorReferences", resolve_entry_reference)
...
entry_details_by_shard = Hash.new { |hash, key| hash[key] = {} }
entries.each do |entry|
  entry_details_by_shard[entry_detail_shard_key(entry)][entry["slug"]] = {
...
search_index_filename = "search-index.#{catalog_version}.json"
search_index_path = "/catalog/#{search_index_filename}"
search_index = {
  "catalogVersion" => catalog_version,
  "generatedAt" => generated_at,
...
web_output = output.merge(
  ...
  "entries" => entries.map do |entry|
    entry.reject do |key, _value|
      %w[
        askNext body example misuse note practicalMeaning searchText
        categorySlugs related relatedSlugs seeAlso resolvedSeeAlso translations
        url vendorReferences resolvedVendorReferences warningLabel whyExists
        tags misunderstoodScore
      ].include?(key)
    end
  end,
)
...
mobile_manifest = {
  ...
  "compatibility" => {
    ...
    "minimumAppleBuildNumber" => nil,
    "minimumAndroidVersionCode" => nil,
    "updateStatus" => "none",
    "upgradeMessage" => nil,
  },
}
...
FileUtils.rm_rf(DETAILS_SHARD_DIR)
FileUtils.mkdir_p(DETAILS_SHARD_DIR)
entry_details_by_shard.keys.sort.each do |shard_key|
  File.write(
    File.join(DETAILS_SHARD_DIR, "#{shard_key}.json"),
    "#{JSON.generate(entry_details_by_shard[shard_key])}\n",
  )
end
File.write(
  DETAILS_SHARD_LOADER_FILE,
  entry_detail_shard_loader_source(entry_details_by_shard.keys.sort),
)
...
Dir.glob(File.join(PUBLIC_CATALOG_DIR, "search-index.*.json")).each do |path|
  File.delete(path) unless File.basename(path) == search_index_filename
end

File.write(versioned_catalog_path, snapshot_text)
File.write(File.join(PUBLIC_CATALOG_DIR, search_index_filename), "#{JSON.generate(search_index)}\n")
