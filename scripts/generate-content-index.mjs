...
const maxEntryDetailShardBytes = 145 * 1024;
...
function splitEntryDetailShard(baseKey, shard) {
  const entries = Object.entries(shard);
  const chunks = [];
  let currentChunk = {};

  for (const [slug, detail] of entries) {
    const candidateChunk = { ...currentChunk, [slug]: detail };

    if (
      Object.keys(currentChunk).length > 0 &&
      serializedByteLength(candidateChunk) > maxEntryDetailShardBytes
    ) {
      chunks.push(currentChunk);
      currentChunk = { [slug]: detail };
    } else {
  ...
  return {
    key: baseKey,
    files: chunks.map((details, index) => ({
      key: `${baseKey}-${index + 1}`,
      details,
    })),
  };
...
function buildEntryDetailShardPlans(entryDetailsByShard) {
  return [...entryDetailsByShard.entries()]
    .sort(([leftKey], [rightKey]) => leftKey.localeCompare(rightKey))
    .map(([baseKey, shard]) => splitEntryDetailShard(baseKey, shard));
}

function buildEntryDetailShardLoaderSource(shardPlans) {
  const entries = shardPlans
    .map(({ key, files }) => {
      if (files.length === 1 && files[0].key === key) {
        return `  ${JSON.stringify(key)}: () => import(${JSON.stringify(`./entry-details/${key}.json`)}),`;
      }

      const imports = files
        .map(({ key: fileKey }) => `      import(${JSON.stringify(`./entry-details/${fileKey}.json`)})`)
        .join(",\n");
...
async function buildEntryIndex() {
  ...
  const detailShardPlans = buildEntryDetailShardPlans(entryDetailsByShard);
  const detailShardFiles = detailShardPlans.flatMap((plan) => plan.files);
  ...
  await Promise.all(
    detailShardFiles.map((shard) =>
      fs.writeFile(
        path.join(detailsShardDirectory, `${shard.key}.json`),
        `${JSON.stringify(shard.details)}\n`,
        "utf8",
      ),
    ),
  );
  await fs.writeFile(
    detailsShardLoaderFile,
    buildEntryDetailShardLoaderSource(detailShardPlans),
    "utf8",
  );
